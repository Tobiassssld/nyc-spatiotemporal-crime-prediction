...
For day \(t\), region \(r\), and feature \(m\), the prediction is

\[
\hat{y}_{t,r} = \sum_m X_{t,r,m} W_{t,r,m},
\]

where \(W_{t,r,:}\) is a time- and region-specific feature-weight vector.

The baseline objective contains three components:

1. prediction error on the training period;
2. temporal smoothness between adjacent weight tensors \(W_t\) and \(W_{t-1}\);
3. spatial smoothness between neighboring regions.

The implementation is TCP-inspired rather than an exact reproduction of the original optimization procedure. It uses squared L2 regularization, polygon-adjacency spatial coupling, and gradient-based optimization with Adam.

### Domain-informed extensions

#### 1. Weekly temporal coupling

An additional regularization term encourages

\[
W_t \approx W_{t-7},
\]

allowing the model to represent recurring weekday-level behavior.

#### 2. Day-type spatial penalties

Instead of using one spatial smoothness parameter for every day, the extended model learns separate regularization strengths for:

- Monday–Thursday;
- Friday;
- Saturday–Sunday.

This allows spatial dependence to vary with weekly activity patterns.

#### 3. Holiday offsets

A binary US-holiday indicator is used to learn an additive feature-weight offset:

\[
W^{\mathrm{eff}}_{t,r,:} = W_{t,r,:} + H_t B.
\]

The offset is shared across regions and regularized with a ridge penalty.

### Alpha-stage forecasting

The model learns time-varying weights during training. For validation and test periods, future weights are forecast rather than directly optimized.

For each region, the alpha stage estimates autoregressive coefficients from historical model weights and recursively forecasts the weight tensors required for unseen days. The default experiment uses a **28-day history window** with ridge regularization.

## Evaluation

The default experiment uses one chronological temporal holdout over the 365-day sequence:

| Split | Size | Period |
| --- | ---: | --- |
| Training | 255 days | 2012-07-01 to 2013-03-12 |
| Validation | 54 days | 2013-03-13 to 2013-05-05 |
| Test | 56 days | 2013-05-06 to 2013-06-30 |

Hyperparameters are selected on the validation period. The selected configuration is then retrained on the combined training and validation data before evaluation on the held-out test period.

The main metric is **average RMSE across regions (aRMSE)**:
...
\frac{1}{T}
\sum_{t=1}^{T}
(\hat{y}_{t,r}-y_{t,r})^2
}.
\]

Lower values are better.

The split utility also supports walk-forward evaluation when shorter train/validation/test windows are configured.
